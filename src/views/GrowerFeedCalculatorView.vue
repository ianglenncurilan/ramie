...
async function saveFormulation() {
  try {
    ...
    // Check Carbs total (should be exactly 50 for Grower)
    const carbsTotal =
      uiCategories.value
    ...
    if (carbsTotal !== 50) {
      validationErrors.push(`Total carbohydrates must be exactly 50% (current: ${carbsTotal}%)`)
    }

    // Check Protein total (should be exactly 50 for Grower)
    const proteinTotal =
      uiCategories.value
    ...
    if (proteinTotal !== 50) {
      validationErrors.push(`Total protein must be exactly 50% (current: ${proteinTotal}%)`)
    }
