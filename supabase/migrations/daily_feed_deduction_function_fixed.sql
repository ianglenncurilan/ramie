-- Daily Feed Deduction Function - FIXED VERSION
...
-- First, ensure we have necessary tables and columns
...
-- Uses the actual feed_inventory table structure
...
DECLARE
  ...
  cost_per_kg DECIMAL;
BEGIN
  -- Get total cost and stock for category from feed_inventory
  -- Using the actual columns that exist: total_cost, starter_stock, grower_stock, finisher_stock
  SELECT 
...
DECLARE
  ...
  daily_cost DECIMAL;
  deduction_date DATE;
  total_deductions JSONB := '{"starter": 0, "grower": 0, "finisher": 0}'::JSONB;
  category_deductions JSONB;
  ...
  LOOP
    BEGIN
      ...
      category_deductions := jsonb_set(
        category_deductions,
        ARRAY[feed_category],
        COALESCE((category_deductions->>feed_category)::DECIMAL, 0) + daily_kg
      );
